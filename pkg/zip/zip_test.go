			...
			for name, content := range files {
				...
				if name[len(name)-1] == '/' {
					...
					if info, err := os.Stat(path); err != nil || !info.IsDir() {
						t.Errorf("Directory %s was not created properly", name)
					}
					continue
				}
...
func TestSanitizeArchivePath(t *testing.T) {
	...
	// Create test zip with path traversal attempt
	files := map[string][]byte{
	...
	tempDir, err := os.MkdirTemp("", "zip_security_test_*")
	if err != nil {
		t.Fatal("Failed to create temp dir:", err)
	}
	defer os.RemoveAll(tempDir)

	// Test unzip with potential path traversal
	...
	if err != nil {
		t.Fatal("Failed to read parent directory:", err)
	}

	for _, entry := range entries {
		if entry.Name() != filepath.Base(tempDir) {
