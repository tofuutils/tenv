...
func main() {
	...
	builders := map[string]builder.BuilderFunc{
		config.TofuName:       builder.BuildTofuManager,
		config.TerraformName:  builder.BuildTfManager,
		config.TerragruntName: builder.BuildTgManager,
		config.AtmosName:      builder.BuildAtmosManager,
	}
...
func initRootCmd(conf *config.Config, builders map[string]builder.BuilderFunc, gruntParser terragruntparser.TerragruntParser) *cobra.Command {
	...
	tofuManager := builders[config.TofuName](conf, gruntParser)
	...
	initSubCmds(tfCmd, conf, builders[config.TerraformName](conf, gruntParser), tfParams)
	...
	initSubCmds(tgCmd, conf, builders[config.TerragruntName](conf, gruntParser), tgParams)
	...
	initSubCmds(atmosCmd, conf, builders[config.AtmosName](conf, gruntParser), atmosParams)
...
func manageHiddenCallCmd(conf *config.Config, builders map[string]builder.BuilderFunc, gruntParser terragruntparser.TerragruntParser) {
	if len(os.Args) < 3 || os.Args[1] != config.CallSubCmd {
		return
	}
	...
	} else if calledNamed == config.AgnosticCmdName {
