...
func (r *TerraformRetriever) ListReleases() ([]string, error) {
	...
	if r.conf.Tf.GetListMode() == htmlretriever.ListModeHTML {
		return htmlretriever.ListReleases(baseURL, r.conf.Tf.Data, r.conf.DisplayVerbose)
	}
...
func (r *TerraformRetriever) checkSumAndSig(fileName string, data []byte, downloadSumsURL string, downloadSumsSigURL string) error {
	dataSums, err := download.Bytes(downloadSumsURL, r.conf.DisplayVerbose)
	if err != nil {
		return err
	}
	...
	dataSumsSig, err := download.Bytes(downloadSumsSigURL, r.conf.DisplayVerbose)
	if err != nil {
		return err
	}
